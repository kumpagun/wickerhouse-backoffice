...
<div class="row breadcrumbs-top">
  <div class="breadcrumb-wrapper col-12">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><a href="{{ route('quiz_index', ['id' => $quiz_group->_id]) }}">แบบทดสอบหลังเรียน EP : {{ CourseClass::get_episode_name($quiz_group->episode_id) }}</a></li>
    ...
    <div class="card">
      ...
      <div class="card-content">
        <div class="card-body overflow-hidden">
          <form id="form" class="form" action="{{ route('quiz_store') }}" method="POST">
            @csrf
            ...
            <div class="form-body">
              ...
              @foreach ($choice_number as $number)
                <fieldset class="form-group floating-label-form-group mb-3">
                  ...
                  <input type="radio" name="answer_key" id="answer-{{$number}}" value={{$number}} @if($quiz->answer_key==$number) checked @endif>
                  <label for="answer-{{$number}}">{{ $number + 1 }}.</label>
                  <div id="choice_{{$number}}">{!! $quiz->choice[$number]['title'] !!}</div>
                </fieldset>
              @endforeach
            </div>
            <div>
