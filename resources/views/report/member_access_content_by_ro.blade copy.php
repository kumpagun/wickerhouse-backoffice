...
@php $title = strtoupper('member access content by ro'); @endphp
        ...
        <div class="card-block card-dashboard">
          <div class="row search-menu">
            <form action="{{route('report_member_access_content_by_RO')}}" method="POST">
              {{ csrf_field() }}
              <div class="col-xs-12 col-md-8 offset-md-4 text-xs-right">
                <div class="row d-flex align-items-baseline">
                  <label class="col-xs-4 text-xs-right"> Training List</label>
                  <div class="col-xs-8">
          ...
          <div class="table-responsive">
            <table class="table table-bordered">
              <thead>
                <tr class="">
                  <th colspan="1" class="align-middle text-xs-center head-table">Count of Status </th>
                  <th colspan="5" class="align-middle text-xs-center">Status</th>
                </tr>
                <tr class="second-row">
                  <th class="align-middle text-xs-center">DeptName</th>
                  <th class="align-middle text-xs-center">เข้าเรียน</th>
                  <th class="align-middle text-xs-center">เข้าเรียน(ผ่าน)</th>
                  <th class="align-middle text-xs-center">เข้าเรียน(ไม่ผ่าน)</th>
                  <th class="align-middle text-xs-center">ไม่เข้าเรียน</th>
                  <th class="align-middle text-xs-center">Total</th>
                </tr>
              </thead>
              <tbody>
                @if(count($datas))
                ...
                @else
                  <tr>
                    <td class="text-xs-center" colspan="6">ไม่มีข้อมูล</td>
                  </tr>
                @endif
              </tbody>
              @if(count($datas))
              <tfoot>
                <tr>
                  ...
                  <td class="text-xs-right"><strong>{{ number_format($data_total['user_active']) }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($data_total['user_active_passing_score']) }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($data_total['user_active_not_passing_score']) }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($data_total['user_inactive']) }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($data_total['user_active'] + $data_total['user_inactive']) }}</strong></td>
                </tr>
                @php
                ...
                @endphp
                <tr>
                  <td class="text-xs-center"><Strong>Total Percent</Strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($total_user_active_percent,2).'%' }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($total_user_active_passing_percent,2).'%' }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($total_user_active_not_passing_percent,2).'%' }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($total_inactive_percent,2).'%' }}</strong></td>
                  <td class="text-xs-right"><strong>{{ number_format($total_user_percent,2).'%' }}</strong></td>
                </tr>
              </tfoot>
              @endif
            </table>
...
@section('style')
  ...
  <link rel="stylesheet" type="text/css" href="{{ asset('stack-admin/app-assets/fonts/feather/style.min.css') }}">
  <link rel="stylesheet" type="text/css" href="{{ asset('stack-admin/app-assets/fonts/font-awesome/css/font-awesome.min.css') }}">
  <link rel="stylesheet" type="text/css" href="{{ asset('stack-admin/app-assets/fonts/flag-icon-css/css/flag-icon.min.css') }}">
  <link rel="stylesheet" type="text/css" href="{{ asset('stack-admin/app-assets/vendors/css/extensions/pace.css') }}">
